...
// FileName Currency_script.js: Currency Javascript file
...
var CurrencyScript = (function(){
	...
	//Url
	var URL="/CurrencyDataHandler";

	//table relation attributes
	var REL = "CurrencyDetails";
	var FORM_ID = "edit_currency_form";
	...
	var INDEX = {
		CHECKBOX : value++, 		// 0
		CURRENCY_ID : value++,		// 1
		CURRENCY_SYMBOL : value++,		// 2
		CURRENCY_WORD : value++,		// 3
		CURRENCY_DECIMAL : value++,		// 4
		CONVERSION_RATE_WITH_USD : value++		// 5
	};
	...
	var LABEL = {
		...
		CONVERSION_RATE_WITH_USD : "Conversion Rate With Usd"
	};
	...
	var DEFAULT = {
		...
		CURRENCY_WORD : "",
		CURRENCY_DECIMAL : "",
		CONVERSION_RATE_WITH_USD : 0.0
	};

	//-----------------------------Form Elements------------------------------------
	var FORM_FIELD = {

		CURRENCY_ID : "#"+FORM_ID+" input[name=currency_id]",
		CURRENCY_SYMBOL : "#"+FORM_ID+" input[name=currency_symbol]",
		CURRENCY_WORD : "#"+FORM_ID+" input[name=currency_word]",
		CURRENCY_DECIMAL : "#"+FORM_ID+" input[name=currency_decimal]",
		CONVERSION_RATE_WITH_USD : "#"+FORM_ID+" input[name=conversion_rate_with_usd]"
	};
	/*------------------------- Load Dependents---------------------------------------*/
...
//	bind event handlers when form is opened
	...
	function createTable(json, _row_index){
		...
		var aoColumnDefs_render = [

					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.CURRENCY_ID ] ); },
								"aTargets": [ INDEX.CURRENCY_ID ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.CURRENCY_SYMBOL ] ); },
								"aTargets": [ INDEX.CURRENCY_SYMBOL ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.CURRENCY_WORD ] ); },
								"aTargets": [ INDEX.CURRENCY_WORD ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.CURRENCY_DECIMAL ] ); },
								"aTargets": [ INDEX.CURRENCY_DECIMAL ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.CONVERSION_RATE_WITH_USD ] ); },
								"aTargets": [ INDEX.CONVERSION_RATE_WITH_USD ]
					}
					];
		...
		var aoColumns_headings = [
						...
						,{	"sTitle": json.headings[INDEX.CURRENCY_ID-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
						,{	"sTitle": json.headings[INDEX.CURRENCY_SYMBOL-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
						,{	"sTitle": json.headings[INDEX.CURRENCY_WORD-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
						,{	"sTitle": json.headings[INDEX.CURRENCY_DECIMAL-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
						,{	"sTitle": json.headings[INDEX.CONVERSION_RATE_WITH_USD-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
					];
	...
	function addTableEvents(table_id){
		...
		if(userRole){
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.EDIT_CURRENCY) == true ){ //edit enabled : permissions
	...
	function attachToolButtonEvents(table_id, button_panel_id){
		...
		var add_button_id = "#"+button_panel_id+' button.add_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.ADD_CURRENCY) == true ){
		...
		var edit_button_id = "#"+button_panel_id+' button.edit_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.EDIT_CURRENCY) == true ){
		...
		var remove_button_id = "#"+button_panel_id+' button.remove_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.DELETE_CURRENCY) == true ){
...
/* Enable as per requirement
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.CURRENCY_ID), G_ERROR.MSG.empty_error+LABEL.CURRENCY_ID);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.CURRENCY_SYMBOL), G_ERROR.MSG.empty_error+LABEL.CURRENCY_SYMBOL);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.CURRENCY_WORD), G_ERROR.MSG.empty_error+LABEL.CURRENCY_WORD);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.CURRENCY_DECIMAL), G_ERROR.MSG.empty_error+LABEL.CURRENCY_DECIMAL);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.CONVERSION_RATE_WITH_USD), G_ERROR.MSG.empty_error+LABEL.CONVERSION_RATE_WITH_USD);
*/
		return bValid;
	}
	function setFormDefaults() {
		DEFAULT.CURRENCY_ID = getNewId(getStorageData(), (INDEX.CURRENCY_ID));
		$(FORM_FIELD.CURRENCY_ID).val(DEFAULT.CURRENCY_ID);
		$(FORM_FIELD.CURRENCY_SYMBOL).val(DEFAULT.CURRENCY_SYMBOL);
		$(FORM_FIELD.CURRENCY_WORD).val(DEFAULT.CURRENCY_WORD);
		$(FORM_FIELD.CURRENCY_DECIMAL).val(DEFAULT.CURRENCY_DECIMAL);
		$(FORM_FIELD.CONVERSION_RATE_WITH_USD).val(DEFAULT.CONVERSION_RATE_WITH_USD);
	}
	function popUpAddForm(field) {
		...
		displayForm("Add New Currency", INSERT_DATA, 0);
	}

	function popUpEditForm(data, row_index) {

		$(FORM_FIELD.CURRENCY_ID).val(data[INDEX.CURRENCY_ID-1]);
		$(FORM_FIELD.CURRENCY_SYMBOL).val(data[INDEX.CURRENCY_SYMBOL-1]);
		$(FORM_FIELD.CURRENCY_WORD).val(data[INDEX.CURRENCY_WORD-1]);
		$(FORM_FIELD.CURRENCY_DECIMAL).val(data[INDEX.CURRENCY_DECIMAL-1]);
		$(FORM_FIELD.CONVERSION_RATE_WITH_USD).val(data[INDEX.CONVERSION_RATE_WITH_USD-1]);
		displayForm("Edit Currency", UPDATE_DATA, row_index);
	}

	function getFormDataAsJson(){
		var jsonData = {
			currency_id : ($(FORM_FIELD.CURRENCY_ID).val()),
			currency_symbol : ($(FORM_FIELD.CURRENCY_SYMBOL).val()),
			currency_word : ($(FORM_FIELD.CURRENCY_WORD).val()),
			currency_decimal : ($(FORM_FIELD.CURRENCY_DECIMAL).val()),
			conversion_rate_with_usd : parseFloat($(FORM_FIELD.CONVERSION_RATE_WITH_USD).val())
		};
	...
	function saveFormData(mode, row_index) {
		...
		com.send({ callback: function(status, response){
			...
			if( status && response.data.status_code >= 0 && mode==INSERT_DATA)
				dialogMessage="Currency has been successfully added ("/*+jsonData.name*/ +" Id:"+response.data.id+")";
			else if( status && response.data.status_code >= 0 && mode==UPDATE_DATA)
				dialogMessage="Currency has been successfully updated("/*+jsonData.name*/+" Id:"+response.data.id+")";
			else
...
//				check whether the delete operation was successful
				if( response.data.status_code == G_ERROR.CODE.SUCCESS ){
					...
					dialogMessage="Currency has been successfully deleted :"+response.data.rows_deleted+" row(s)";
				...
				if( row_data_arr.length > 1)
					dialogMessage = "Cannot delete Currency. There are some dependencies for this Currency.";
				else
					dialogMessage = "Cannot delete Currency. There are some dependencies for this Currency.";
	...
	function fetchData(option){
		...
		if(option && option.currency_id){
			$.each(option.currency_id, function(inx, val){
				_url += '&currency_id='+val;
			});
		}
	...
	function populateInfo(elem, select_id, callback) {
		var  id_index = INDEX.CURRENCY_ID, name_index = getNameIdx();
			...
			fetchData({
				callback: function(status, response){
					if( status && response.data.values){
						...
						console.log('CURRENCY_DATA is empty : selected_id=' +select_id );
					}
					if(callback)
						callback();
				}
	...
	function execMenuAction(id){
		...
		case 2:
			...
			break;	
		default:
			log('Currency Action', 'unknown parameter');
		}
	}
	function getData(){
