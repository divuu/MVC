...
// FileName LookupGroup_script.js: LookupGroup Javascript file
...
var LookupGroupScript = (function(){
	...
	//Url
	var URL="/LookupGroupDataHandler";

	//table relation attributes
	var REL = "LookupGroupDetails";
	var FORM_ID = "edit_lookup_group_form";
	...
	var INDEX = {
		CHECKBOX : value++, 		// 0
		LOOKUP_GROUP_ID : value++,		// 1
		LOOKUP_GROUP_NAME : value++		// 2
	};
	...
	var LABEL = {
		...
		LOOKUP_GROUP_NAME : "Lookup Group Name"
	};
	...
	var FORM_FIELD = {
		...
		LOOKUP_GROUP_ID : "#"+FORM_ID+" input[name=lookup_group_id]",
		LOOKUP_GROUP_NAME : "#"+FORM_ID+" input[name=lookup_group_name]"
	};
	/*------------------------- Load Dependents---------------------------------------*/
...
//	bind event handlers when form is opened
	...
	function createTable(json, _row_index){
		...
		var aoColumnDefs_render = [

					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.LOOKUP_GROUP_ID ] ); },
								"aTargets": [ INDEX.LOOKUP_GROUP_ID ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.LOOKUP_GROUP_NAME ] ); },
								"aTargets": [ INDEX.LOOKUP_GROUP_NAME ]
					}
					];
		...
		var aoColumns_headings = [
						...
						,{	"sTitle": json.headings[INDEX.LOOKUP_GROUP_NAME-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
					];
	...
	function addTableEvents(table_id){
		...
		if(userRole){
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.EDIT_LOOKUP_GROUP) == true ){ //edit enabled : permissions
	...
	function attachToolButtonEvents(table_id, button_panel_id){
		...
		var add_button_id = "#"+button_panel_id+' button.add_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.ADD_LOOKUP_GROUP) == true ){
		...
		var edit_button_id = "#"+button_panel_id+' button.edit_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.EDIT_LOOKUP_GROUP) == true ){
		...
		var remove_button_id = "#"+button_panel_id+' button.remove_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.DELETE_LOOKUP_GROUP) == true ){
...
/* Enable as per requirement
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.LOOKUP_GROUP_ID), G_ERROR.MSG.empty_error+LABEL.LOOKUP_GROUP_ID);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.LOOKUP_GROUP_NAME), G_ERROR.MSG.empty_error+LABEL.LOOKUP_GROUP_NAME);
*/
		return bValid;
	}
	function setFormDefaults() {
		DEFAULT.LOOKUP_GROUP_ID = getNewId(getStorageData(), (INDEX.LOOKUP_GROUP_ID));
		$(FORM_FIELD.LOOKUP_GROUP_ID).val(DEFAULT.LOOKUP_GROUP_ID);
		$(FORM_FIELD.LOOKUP_GROUP_NAME).val(DEFAULT.LOOKUP_GROUP_NAME);
	}
	function popUpAddForm(field) {
		...
		displayForm("Add New Lookup Group", INSERT_DATA, 0);
	}

	function popUpEditForm(data, row_index) {

		$(FORM_FIELD.LOOKUP_GROUP_ID).val(data[INDEX.LOOKUP_GROUP_ID-1]);
		$(FORM_FIELD.LOOKUP_GROUP_NAME).val(data[INDEX.LOOKUP_GROUP_NAME-1]);
		displayForm("Edit Lookup Group", UPDATE_DATA, row_index);
	}

	function getFormDataAsJson(){
		var jsonData = {
			lookup_group_id : ($(FORM_FIELD.LOOKUP_GROUP_ID).val()),
			lookup_group_name : ($(FORM_FIELD.LOOKUP_GROUP_NAME).val())
		};
		return jsonData;
	}
	...
	function saveFormData(mode, row_index) {
		...
		com.send({ callback: function(status, response){
			...
			if( status && response.data.status_code >= 0 && mode==INSERT_DATA)
				dialogMessage="LookupGroup has been successfully added ("+jsonData.name+" Id:"+response.data.id+")";
			else if( status && response.data.status_code >= 0 && mode==UPDATE_DATA)
				dialogMessage="LookupGroup has been successfully updated("+jsonData.name+" Id:"+response.data.id+")";
			else
	...
	function deleteRows(row_data_arr){
		var row_id_arr = [];
		var key_column_index = [ INDEX.LOOKUP_GROUP_ID ];
		row_id_arr = getArrayFromArray({ index : key_column_index, data_arr : row_data_arr});

		var jsonData = {
				del_row_id_arr : row_id_arr
//			,lookup_group_id : getLookupGroupId()	// ADD ANY OTHER CONDITION (IF ANY)
		};
		var inData = JSON.stringify(jsonData);
...
//				check whether the delete operation was successful
				if( response.data.status_code == G_ERROR.CODE.SUCCESS ){
					...
					dialogMessage="Lookup Group has been successfully deleted :"+response.data.rows_deleted+" row(s)";
				...
				if( row_data_arr.length > 1)
					dialogMessage = "Cannot delete Lookup Group. There are some dependencies for this Lookup Group.";
				else
					dialogMessage = "Cannot delete Lookup Group. There are some dependencies for this Lookup Group.";
	...
	function fetchData(option){
		...
		if(option && option.lookup_group_id){
			$.each(option.lookup_group_id, function(inx, val){
				_url += '&lookup_group_id='+val;
			});
		}
	...
	function populateInfo(elem, select_id, callback) {
		var  id_index = INDEX.LOOKUP_GROUP_ID, name_index = getNameIdx();
			...
			fetchData({
				callback: function(status, response){
					if( status && response.data.values){
						...
						console.log('LOOKUP_GROUP_DATA is empty : selected_id=' +select_id );
					}
					if(callback)
						callback();
				}
	...
	function execMenuAction(id){
		...
		case 1:
			...
			break;
		case 2:
			LookupGroupSendEmail();
			break;	
		default:
			log('Lookup Group Action', 'unknown parameter');
		}
	}
	function getData(){
