...
// FileName Lookup_script.js: Lookup Javascript file

// Author : Yashasvi Sinha | JRC
// Description : Jeeva Vedic v1.0


////////////////////////////////////////////////////////////////////////////


var LookupScript = (function(){
	...
	//Url
	var URL="/LookupDataHandler";

	//table relation attributes
	var REL = "LookupDetails";
	var FORM_ID = "edit_lookup_form";
	...
	var INDEX = {
		CHECKBOX : value++, 		// 0
		LOOKUP_ID : value++,		// 1
		LOOKUP_GROUP_ID : value++,		// 2
		TABLE_NAME : value++,		// 3
		COLUMN_NAME : value++,		// 4
		LOOKUP_NAME : value++,		// 5
		LOOKUP_VALUE : value++		// 6
	};
	...
	var FORM_FIELD = {
		...
		LOOKUP_ID : "#"+FORM_ID+" input[name=lookup_id]",
		LOOKUP_GROUP_ID : "#"+FORM_ID+" select[name=lookup_group_id]",
		TABLE_NAME : "#"+FORM_ID+" input[name=table_name]",
		COLUMN_NAME : "#"+FORM_ID+" input[name=column_name]",
		LOOKUP_NAME : "#"+FORM_ID+" input[name=lookup_name]",
		LOOKUP_VALUE : "#"+FORM_ID+" input[name=lookup_value]"
	};
	/*------------------------- Load Dependents---------------------------------------*/
...
//	bind event handlers when form is opened
	...
	function loadTabContent(){
		...
		var lookupGroup = LookupGroupScript.getInstance();
		lookupGroup.getData();
	...
	function createTable(json, _row_index){
		...
		var aoColumnDefs_render = [

					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.LOOKUP_ID ] ); },
								"aTargets": [ INDEX.LOOKUP_ID ]
					},
					{
					"fnRender": function ( o, val ) {
						var lookupGroupName = getValueById( o.aData[ INDEX.LOOKUP_GROUP_ID ], lookupGroup );
						return lookupGroupName;
						},
						"aTargets": [ INDEX.LOOKUP_GROUP_ID ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.TABLE_NAME ] ); },
								"aTargets": [ INDEX.TABLE_NAME ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.COLUMN_NAME ] ); },
								"aTargets": [ INDEX.COLUMN_NAME ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.LOOKUP_NAME ] ); },
								"aTargets": [ INDEX.LOOKUP_NAME ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.LOOKUP_VALUE ] ); },
								"aTargets": [ INDEX.LOOKUP_VALUE ]
					}
					];
		...
		var aoColumns_headings = [
						...
						,{	"sTitle": json.headings[INDEX.LOOKUP_ID-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
						,{	"sTitle": json.headings[INDEX.LOOKUP_GROUP_ID-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
						,{	"sTitle": json.headings[INDEX.TABLE_NAME-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
						,{	"sTitle": json.headings[INDEX.COLUMN_NAME-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
						,{	"sTitle": json.headings[INDEX.LOOKUP_NAME-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
						,{	"sTitle": json.headings[INDEX.LOOKUP_VALUE-1] , "sClass": "center", "sType": "string", "bSearchable": true, "bVisible": true }
					];
	...
	function addTableEvents(table_id){
		...
		if(userRole){
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.EDIT_LOOKUP) == true ){ //edit enabled : permissions
	...
	function attachToolButtonEvents(table_id, button_panel_id){
		...
		var add_button_id = "#"+button_panel_id+' button.add_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.ADD_LOOKUP) == true ){
		...
		var edit_button_id = "#"+button_panel_id+' button.edit_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.EDIT_LOOKUP) == true ){
		...
		var remove_button_id = "#"+button_panel_id+' button.remove_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.DELETE_LOOKUP) == true ){
...
/* Enable as per requirement
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.LOOKUP_ID), G_ERROR.MSG.empty_error+LABEL.LOOKUP_ID);
		bValid = bValid && fv.checkEmptySelect($(FORM_FIELD.LOOKUP_GROUP_ID), G_ERROR.MSG.empty_error_selectbox+LABEL.LOOKUP_GROUP_ID);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.TABLE_NAME), G_ERROR.MSG.empty_error+LABEL.TABLE_NAME);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.COLUMN_NAME), G_ERROR.MSG.empty_error+LABEL.COLUMN_NAME);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.LOOKUP_NAME), G_ERROR.MSG.empty_error+LABEL.LOOKUP_NAME);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.LOOKUP_VALUE), G_ERROR.MSG.empty_error+LABEL.LOOKUP_VALUE);
*/
		return bValid;
	}
	function setFormDefaults() {
		DEFAULT.LOOKUP_ID = getNewId(getStorageData(), (INDEX.LOOKUP_ID));
		$(FORM_FIELD.LOOKUP_ID).val(DEFAULT.LOOKUP_ID);
		var lookupGroup = LookupGroupScript.getInstance();
		lookupGroup.populateInfo( $(FORM_FIELD.LOOKUP_GROUP_ID), DEFAULT.LOOKUP_GROUP_ID );
		$(FORM_FIELD.TABLE_NAME).val(DEFAULT.TABLE_NAME);
		$(FORM_FIELD.COLUMN_NAME).val(DEFAULT.COLUMN_NAME);
		$(FORM_FIELD.LOOKUP_NAME).val(DEFAULT.LOOKUP_NAME);
		$(FORM_FIELD.LOOKUP_VALUE).val(DEFAULT.LOOKUP_VALUE);
	}
	function popUpAddForm(field) {
		...
		displayForm("Add New Lookup", INSERT_DATA, 0);
	}

	function popUpEditForm(data, row_index) {

		$(FORM_FIELD.LOOKUP_ID).val(data[INDEX.LOOKUP_ID-1]);
		var lookupGroup = LookupGroupScript.getInstance();
		lookupGroup.populateInfo( $(FORM_FIELD.LOOKUP_GROUP_ID), DEFAULT.LOOKUP_GROUP_ID, data[INDEX.LOOKUP_GROUP_ID-1] );
		$(FORM_FIELD.TABLE_NAME).val(data[INDEX.TABLE_NAME-1]);
		$(FORM_FIELD.COLUMN_NAME).val(data[INDEX.COLUMN_NAME-1]);
		$(FORM_FIELD.LOOKUP_NAME).val(data[INDEX.LOOKUP_NAME-1]);
		$(FORM_FIELD.LOOKUP_VALUE).val(data[INDEX.LOOKUP_VALUE-1]);
		displayForm("Edit Lookup", UPDATE_DATA, row_index);
	}

	function getFormDataAsJson(){
		var jsonData = {
			lookup_id : ($(FORM_FIELD.LOOKUP_ID).val()),
			lookup_group_id : ($(FORM_FIELD.LOOKUP_GROUP_ID).val()),
			table_name : ($(FORM_FIELD.TABLE_NAME).val()),
			column_name : ($(FORM_FIELD.COLUMN_NAME).val()),
			lookup_name : ($(FORM_FIELD.LOOKUP_NAME).val()),
			lookup_value : ($(FORM_FIELD.LOOKUP_VALUE).val())
		};
		return jsonData;
	}
	...
	function saveFormData(mode, row_index) {
		...
		com.send({ callback: function(status, response){
			...
			if( status && response.data.status_code >= 0 && mode==INSERT_DATA)
				dialogMessage="Lookup has been successfully added ("+jsonData.name+" Id:"+response.data.id+")";
			else if( status && response.data.status_code >= 0 && mode==UPDATE_DATA)
				dialogMessage="Lookup has been successfully updated("+jsonData.name+" Id:"+response.data.id+")";
			else
...
//				check whether the delete operation was successful
				if( response.data.status_code == G_ERROR.CODE.SUCCESS ){
					...
					dialogMessage="Lookup has been successfully deleted :"+response.data.rows_deleted+" row(s)";
				...
				if( row_data_arr.length > 1)
					dialogMessage = "Cannot delete Lookup. There are some dependencies for this Lookup.";
				else
					dialogMessage = "Cannot delete Lookup. There are some dependencies for this Lookup.";
	...
	function fetchData(option){
		...
		if(option && option.lookup_id){
			$.each(option.lookup_id, function(inx, val){
				_url += '&lookup_id='+val;
			});
		}
...
/*  OLD CODE
		...
		//var _url = ROOT_URL + URL +"?sys_user_id="+ getCurLogUserId();

		if(option && option.lookup_group_id){
//			$.each(option.lookup_id, function(inx, val){
				_url += '&lookup_group_id='+option.lookup_group_id;
//			});
		}
		if(option && option.lookup_id) {
			_url += '&lookup_id='+option.lookup_id;
		}
*/
	...
	function getLookupNameById(lookup_id, namecallback){
		var name = "";
		if(lookup_id != 0) {
			fetchData({
				lookup_id: lookup_id,
				namecallback: function(status, data){
					if( status && data.values){
						values =  data.values;
						name = values[0][4];
					}
					else{
						console.log('LOOKUP_DATA is empty :' + lookup_id);
					}
					if(namecallback)
						namecallback(name);
				}
	...
	/* populate combobox*/
	function populateInfo(elem,  lookup_group_id, select_id, callback) {
		var  id_index = INDEX.LOOKUP_ID, name_index = getNameIdx();
		...
		if(null !== getStorageData()){
			...
			populateCombobox(elem, values, id_index, name_index, lookup_group_id );
			elem.val(select_id);
			if(callback)
				callback();
		}
			...
			fetchData({
				lookup_group_id: lookup_group_id,
				callback: function(status, response){
					if( status && response.data.values){
						...
						console.log('LOOKUP_DATA is empty : selected_id=' +select_id );
					}
					if(callback)
						callback();
				}
	...
	function execMenuAction(id){
		...
		case 2:
			...
			break;	
		default:
			log('Lookup Action', 'unknown parameter');
		}
	}
	function getData(){
	...
	function getIdIdx() {
		...
		return INDEX.LOOKUP_ID;
	}

	function getNameIdx() {
		return INDEX.LOOKUP_NAME;
	}
	...
	function init(){
		return {
			...
			getIdIdx: getIdIdx,
			getLookupNameById: getLookupNameById,
