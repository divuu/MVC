...
// FileName Role_script.js: Role Javascript file
...
var RoleScript = (function(){
	...
	//Url
	var URL="/RoleDataHandler";

	//table relation attributes
	var REL = "RoleDetails";
	var FORM_ID = "edit_role_form";
	...
	var INDEX = {
		...
		NAME : value++,		// 3
		ROLE_WEB_APP_UI_FEATURES : value++,		// 4
		ROLE_MOBILE_APP_UI_FEATURES : value++,		// 5
		ROLE_REPORT_FEATURES : value++,		// 6
		ROLE_MENU : value++,		// 7
		ROLE_SCRIPT : value++,		// 8
		REMARKS : value++		// 9
	};
	...
	var LABEL = {
		...
		NAME : "Name",
		ROLE_WEB_APP_UI_FEATURES : "Role Web App Ui Features",
		ROLE_MOBILE_APP_UI_FEATURES : "Role Mobile App Ui Features",
		ROLE_REPORT_FEATURES : "Role Report Features",
		ROLE_MENU : "Role Menu",
		ROLE_SCRIPT : "Role Script",
		REMARKS : "Remarks"
	};
	...
	var DEFAULT = {
		...
		ORGANIZATION_ID : 0,
		NAME : "",
		ROLE_WEB_APP_UI_FEATURES : "",
		ROLE_MOBILE_APP_UI_FEATURES : "",
		ROLE_REPORT_FEATURES : "",
		ROLE_MENU : "",
		ROLE_SCRIPT : "",
		REMARKS : ""
	};
	...
	var FORM_FIELD = {
		...
		ROLE_ID : "#"+FORM_ID+" input[name=role_id]",
		ORGANIZATION_ID : "#"+FORM_ID+" select[name=organization_id]",
		NAME : "#"+FORM_ID+" input[name=name]",
		ROLE_WEB_APP_UI_FEATURES : "#"+FORM_ID+" textarea[name=role_web_app_ui_features]",
		ROLE_MOBILE_APP_UI_FEATURES : "#"+FORM_ID+" textarea[name=role_mobile_app_ui_features]",
		ROLE_REPORT_FEATURES : "#"+FORM_ID+" textarea[name=role_report_features]",
		ROLE_MENU : "#"+FORM_ID+" textarea[name=role_menu]",
		ROLE_SCRIPT : "#"+FORM_ID+" textarea[name=role_script]",
...
//	bind event handlers when form is opened
	...
	function createTable(json, _row_index){
		...
		var aoColumnDefs_render = [

					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.ROLE_ID ] ); },
					...
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.ROLE_WEB_APP_UI_FEATURES ] ); },
								"aTargets": [ INDEX.ROLE_WEB_APP_UI_FEATURES ]
					},
					{
						"fnRender":	function ( o, val ) {
									return getSmsLink( o.aData[ INDEX.ROLE_MOBILE_APP_UI_FEATURES ] ); },
								"aTargets": [ INDEX.ROLE_MOBILE_APP_UI_FEATURES ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.ROLE_REPORT_FEATURES ] ); },
								"aTargets": [ INDEX.ROLE_REPORT_FEATURES ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.ROLE_MENU ] ); },
								"aTargets": [ INDEX.ROLE_MENU ]
					},
					{
						"fnRender":	function ( o, val ) {
									return myFunction( o.aData[ INDEX.ROLE_SCRIPT ] ); },
								"aTargets": [ INDEX.ROLE_SCRIPT ]
					},
					{
						"fnRender":	function ( o, val ) {
		...
		var aoColumns_headings = [
						...
						,{	"sTitle": json.headings[INDEX.ROLE_WEB_APP_UI_FEATURES-1] , "sClass": "center", "sType": "string", "bSearchable": false, "bVisible": false }
						,{	"sTitle": json.headings[INDEX.ROLE_MOBILE_APP_UI_FEATURES-1] , "sClass": "center", "sType": "string", "bSearchable": false, "bVisible": false }
						,{	"sTitle": json.headings[INDEX.ROLE_REPORT_FEATURES-1] , "sClass": "center", "sType": "string", "bSearchable": false, "bVisible": false }
						,{	"sTitle": json.headings[INDEX.ROLE_MENU-1] , "sClass": "center", "sType": "string", "bSearchable": false, "bVisible": false }
						,{	"sTitle": json.headings[INDEX.ROLE_SCRIPT-1] , "sClass": "center", "sType": "string", "bSearchable": false, "bVisible": false }
	...
	function addTableEvents(table_id){
		...
		if(userRole){
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.EDIT_ROLE) == true ){ //edit enabled : permissions
	...
	function attachToolButtonEvents(table_id, button_panel_id){
		...
		var add_button_id = "#"+button_panel_id+' button.add_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.ADD_ROLE) == true ){
		...
		var edit_button_id = "#"+button_panel_id+' button.edit_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.EDIT_ROLE) == true ){
		...
		var remove_button_id = "#"+button_panel_id+' button.remove_button';
			if( checkRolePermission(userRole, SOFTWARE_FEATURE_CONST.DELETE_ROLE) == true ){
...
/* Enable as per requirement
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.ROLE_ID), G_ERROR.MSG.empty_error+LABEL.ROLE_ID);
		...
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.ROLE_WEB_APP_UI_FEATURES), G_ERROR.MSG.empty_error+LABEL.ROLE_WEB_APP_UI_FEATURES);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.ROLE_MOBILE_APP_UI_FEATURES), G_ERROR.MSG.empty_error+LABEL.ROLE_MOBILE_APP_UI_FEATURES);
		bValid = bValid && fv.validateMobileNo( $(FORM_FIELD.ROLE_MOBILE_APP_UI_FEATURES), country_code, 0 , LABEL.ROLE_MOBILE_APP_UI_FEATURES+G_ERROR.MSG.invalid_mobile_no);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.ROLE_REPORT_FEATURES), G_ERROR.MSG.empty_error+LABEL.ROLE_REPORT_FEATURES);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.ROLE_MENU), G_ERROR.MSG.empty_error+LABEL.ROLE_MENU);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.ROLE_SCRIPT), G_ERROR.MSG.empty_error+LABEL.ROLE_SCRIPT);
		bValid = bValid && fv.checkEmpty($(FORM_FIELD.REMARKS), G_ERROR.MSG.empty_error+LABEL.REMARKS);
*/
		return bValid;
	}
	function setFormDefaults() {
		DEFAULT.ROLE_ID = getNewId(getStorageData(), (INDEX.ROLE_ID));
		$(FORM_FIELD.ROLE_ID).val(DEFAULT.ROLE_ID);
		var organization = OrganizationScript.getInstance();
		organization.populateInfo( $(FORM_FIELD.ORGANIZATION_ID), DEFAULT.ORGANIZATION_ID );
		$(FORM_FIELD.NAME).val(DEFAULT.NAME);
		$(FORM_FIELD.ROLE_WEB_APP_UI_FEATURES).val(DEFAULT.ROLE_WEB_APP_UI_FEATURES);
		$(FORM_FIELD.ROLE_MOBILE_APP_UI_FEATURES).val(DEFAULT.ROLE_MOBILE_APP_UI_FEATURES);
		$(FORM_FIELD.ROLE_REPORT_FEATURES).val(DEFAULT.ROLE_REPORT_FEATURES);
		$(FORM_FIELD.ROLE_MENU).val(DEFAULT.ROLE_MENU);
		$(FORM_FIELD.ROLE_SCRIPT).val(DEFAULT.ROLE_SCRIPT);
	...
	function popUpAddForm(field) {
		...
		displayForm("Add New Role", INSERT_DATA, 0);
	}

	function popUpEditForm(data, row_index) {

		$(FORM_FIELD.ROLE_ID).val(data[INDEX.ROLE_ID-1]);
		var organization = OrganizationScript.getInstance();
		organization.populateInfo( $(FORM_FIELD.ORGANIZATION_ID), data[INDEX.ORGANIZATION_ID-1] );
		$(FORM_FIELD.NAME).val(data[INDEX.NAME-1]);
		$(FORM_FIELD.ROLE_WEB_APP_UI_FEATURES).val(data[INDEX.ROLE_WEB_APP_UI_FEATURES-1]);
		$(FORM_FIELD.ROLE_MOBILE_APP_UI_FEATURES).val(data[INDEX.ROLE_MOBILE_APP_UI_FEATURES-1]);
		$(FORM_FIELD.ROLE_REPORT_FEATURES).val(data[INDEX.ROLE_REPORT_FEATURES-1]);
		$(FORM_FIELD.ROLE_MENU).val(data[INDEX.ROLE_MENU-1]);
		$(FORM_FIELD.ROLE_SCRIPT).val(data[INDEX.ROLE_SCRIPT-1]);
		$(FORM_FIELD.REMARKS).val(data[INDEX.REMARKS-1]);
		displayForm("Edit Role", UPDATE_DATA, row_index);
	}
	...
	function getFormDataAsJson(){
		var jsonData = {
			...
			role_web_app_ui_features : ($(FORM_FIELD.ROLE_WEB_APP_UI_FEATURES).val()),
			role_mobile_app_ui_features : ($(FORM_FIELD.ROLE_MOBILE_APP_UI_FEATURES).val()),
			role_report_features : ($(FORM_FIELD.ROLE_REPORT_FEATURES).val()),
			role_menu : ($(FORM_FIELD.ROLE_MENU).val()),
			role_script : ($(FORM_FIELD.ROLE_SCRIPT).val()),
	...
	function saveFormData(mode, row_index) {
		...
		com.send({ callback: function(status, response){
			...
			if( status && response.data.status_code >= 0 && mode==INSERT_DATA)
				dialogMessage="Role has been successfully added ("/*+jsonData.name*/ +" Id:"+response.data.id+")";
			else if( status && response.data.status_code >= 0 && mode==UPDATE_DATA)
				dialogMessage="Role has been successfully updated("/*+jsonData.name*/+" Id:"+response.data.id+")";
			else
...
//				check whether the delete operation was successful
				if( response.data.status_code == G_ERROR.CODE.SUCCESS ){
					...
					dialogMessage="Role has been successfully deleted :"+response.data.rows_deleted+" row(s)";
				...
				if( row_data_arr.length > 1)
					dialogMessage = "Cannot delete Role. There are some dependencies for this Role.";
				else
					dialogMessage = "Cannot delete Role. There are some dependencies for this Role.";
	...
	function fetchData(option){
		...
		if(option && option.role_id){
			$.each(option.role_id, function(inx, val){
				_url += '&role_id='+val;
			});
		}
	...
	function populateInfo(elem, select_id, callback) {
		var  id_index = INDEX.ROLE_ID, name_index = getNameIdx();
			...
			fetchData({
				callback: function(status, response){
					if( status && response.data.values){
						...
						console.log('ROLE_DATA is empty : selected_id=' +select_id );
					}
					if(callback)
						callback();
				}
	...
	function execMenuAction(id){
		...
		case 2:
			...
			break;	
		default:
			log('Role Action', 'unknown parameter');
		}
	}
	function getData(){
