...
// FileName CityDataHandler.java: CityDataHandler Implementation file
...
@Path("/CityDataHandler")

public class CityDataHandler extends DataHandler {
	...
	public Response getCityList( @QueryParam("organization_id") int whrCondition ) {
		try {

			if( AuthenticationUtil.authenticateUserAccess(request, SoftwareFeature.READ_CITY) == false) {
			...
			CityDbAdaptor dbAdaptor = new CityDbAdaptor();

			String condition = null; //CityDbAdaptor.KEY_ORGANIZATION_ID +"="+ whrCondition;
			String orderBy = CityDbAdaptor.KEY_CITY_ID;
			MLogger.i( MLogger.MOD_DB, "Condition:" + condition);
			ArrayList<City> finalList = dbAdaptor.getList( condition, orderBy );

			if( finalList == null || finalList.size() == 0 ) {

				MLogger.i( MLogger.MOD_DB, "City List is empty: No data to send");
				JSONObject errorObj = new JSONObject();
				errorObj.put("rel", "CityDetails");
				errorObj.put("columns", City.NUM_OF_COLUMNS);
				errorObj.put("headings", City.getHeader());
			...
			jsonObj.put("rel", "CityDetails");
			jsonObj.put("rows", finalList.size());
			jsonObj.put("columns", City.NUM_OF_COLUMNS);
			jsonObj.put("headings", City.getHeader());
			jsonObj.put("values", City.objListToJSONArrayList(finalList));
	...
	public Response getTableHeaders() {
		try {
			if( AuthenticationUtil.authenticateUserAccess(request, SoftwareFeature.READ_ASSIGNED_CITY) == false) {
				JSONObject errorObj = new JSONObject();
				errorObj.put("status_code", ErrorCodes.ERROR_CODE_ACCESS_DENIED);
				return Response.status( Response.Status.NO_CONTENT )
						.entity(errorObj)
			...
			JSONObject jsonObj = new JSONObject();
			jsonObj.put("rel", "CityDetails");
			jsonObj.put("columns", City.NUM_OF_COLUMNS); // number of column in each row
			jsonObj.put("headings",	City.getHeader());   // each column name
	...
	protected Response saveData( boolean insertData, String inputString ) {
		if(insertData == true){
			if( AuthenticationUtil.authenticateUserAccess(request, SoftwareFeature.ADD_CITY) == false) {
				JSONObject errorObj = new JSONObject();
				try {
					errorObj.put("status_code", ErrorCodes.ERROR_CODE_ACCESS_DENIED);
			...
			if( AuthenticationUtil.authenticateUserAccess(request, SoftwareFeature.EDIT_CITY) == false) {
				JSONObject errorObj = new JSONObject();
				try {
					errorObj.put("status_code", ErrorCodes.ERROR_CODE_ACCESS_DENIED);
				} catch (JSONException e) {
		...
		try {
			...
			CityDbAdaptor dbAdaptor = new CityDbAdaptor();
			//obj.mOrganizationId = Integer.valueOf(data.getInt(CityDbAdaptor.KEY_ORGANIZATION_ID));
			String condition = null;
			if( insertData == true ) {
				condition = null; // CityDbAdaptor.KEY_ORGANIZATION_ID+"="+obj.mOrganizationId;
				String fieldName = CityDbAdaptor.KEY_CITY_ID;
				obj.mCityId =  ( 1 + dbAdaptor.runAggregate( "max", fieldName, condition ) );
			}
			else {
				obj.mCityId =  data.getInt( CityDbAdaptor.KEY_CITY_ID );
				condition = CityDbAdaptor.KEY_CITY_ID+"="+obj.mCityId;
			}

			obj.mCity = data.getString(CityDbAdaptor.KEY_CITY).trim();
			obj.mCityAreaCode = data.getString(CityDbAdaptor.KEY_CITY_AREA_CODE).trim();
			obj.mTimeZone = data.getString(CityDbAdaptor.KEY_TIME_ZONE).trim();
			obj.mStateLookupId = data.getInt(CityDbAdaptor.KEY_STATE_LOOKUP_ID);
			obj.mCountryId = data.getInt(CityDbAdaptor.KEY_COUNTRY_ID);
			long status = 0;
	...
	protected long deleteData( String delete_data ) {
		...
		if( AuthenticationUtil.authenticateUserAccess(request, SoftwareFeature.DELETE_CITY) == false) {
		...
		try{
			...
			//int organization_id = data.getInt(CityDbAdaptor.KEY_ORGANIZATION_ID);
			...
			for(int i = 0; i<(length-1);i++){
				...
				condition += "(";
				condition += CityDbAdaptor.KEY_CITY_ID + " = " + id_arr.getString(j) + " ) or ";
			}
			int j = 0;
			JSONArray id_arr = object_id_arr.getJSONArray(length -1);
			condition += "(";

			condition += CityDbAdaptor.KEY_CITY_ID + " = " + id_arr.getString(j++) + " ) ) ";
			//condition += " and " + CityDbAdaptor.KEY_ORGANIZATION_ID + " = " + id_arr.getString(j++) + " ";;

			MLogger.i( MLogger.MOD_DB, "Deleting City: where clause " + condition );
			CityDbAdaptor dbAdaptor = new CityDbAdaptor();
			long noOfRowsDeleted = dbAdaptor.delete(condition);

			if( noOfRowsDeleted < 0 ) {
				MLogger.i( MLogger.MOD_DB, "Deleting City: FAILED, status= " + noOfRowsDeleted + ". Rolling back" );
